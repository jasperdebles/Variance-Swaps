...
# A swap option today expiring at 9/20
# We use call and put options expiring at this date too

AEXputs = read.csv("AEXputs.csv")
View(AEXputs)
AEXcalls = read.csv("AEXcalls.csv")
View(AEXcalls)

S0 <- c(594.93) #spot price today of the AEX 11/7/2019

puts  <- matrix(AEXputs$Strike)    #available put strike prices for 09/2020

vol_put     <- matrix(AEXputs$IVM)   #implied vols for puts

calls <- matrix(AEXcalls$Strike)          #available call strike prices

vol_call    <- matrix(AEXcalls$IVM) #implied vols for calls


r  <- c( 0.015 )   #approximating current risk free rate 1-year treasury yield 

T  <- c( 1/12*11 ) #maturity of 11 months from now to September

SQ <- c(S0*exp(r*T)) #price which is nearest to forward price


AEXequity_varswap <- VarSwap(S0, puts, calls, vol_put, vol_call, r, T, SQ) 
